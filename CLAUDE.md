...
**The transport layer stays the same.** ATC does not replace Slack, Telegram, or any messaging platform. Human teammates keep working as they do. Agents keep communicating as they do. ATC reads from and writes to those platforms on the operator's behalf, making the underlying platform invisible without removing it.
...
**Post as the operator, not as a bot.** When ATC sends a message back to an agent thread, it posts using the operator's identity — not a bot account. Agents are already trained to respond to the operator. Preserving that trust relationship is not optional.

**No vendor lock-in, ever.** ATC must never require a specific model provider, cloud platform, messaging service, or task management tool to function. Every external dependency is behind an interface. Operators bring their own everything: their own LLM, their own Slack workspace, their own Jira instance. ATC is the engine; they supply the fuel.

**Self-hostable by default.** ATC runs entirely on the operator's machine with zero cloud dependencies required. No account creation, no telemetry, no phoning home. An operator with no internet access should be able to run ATC against a local model and a self-hosted messaging platform and have a fully functional product.
...
ATC has five core components plus an MCP server layer:
...
The context graph is local-first and portable. It belongs to the operator. The SQLite file is directly accessible, exportable, and readable with any SQLite tool. ATC will never move this data to a cloud service without explicit operator action.
...
### 6. MCP Server (ATC as a Tool)
ATC exposes a local MCP server so agents can push status directly to ATC rather than having ATC scrape it from messaging platforms. This is architecturally cleaner for agents that natively support MCP and removes the dependency on a messaging platform as the intermediary.

The MCP server exposes tools that agents can call:
- `atc_report_status(workItemId, status, message)` — agent proactively reports its state
- `atc_request_approval(workItemId, description, options)` — agent requests a human decision
- `atc_complete(workItemId, summary)` — agent marks a work item done

This means ATC works in two modes simultaneously: **passive** (scraping messages from platform adapters) and **active** (receiving direct pushes from MCP-capable agents). As the agent ecosystem matures toward MCP, passive scraping becomes the legacy path and active push becomes the norm. Both are always supported.

---

## Extensibility Model

ATC is designed to be extended without forking. The extension points are:
...
- MCP server with `atc_report_status`, `atc_request_approval`, `atc_complete` tools
...
Add action buttons and free-text reply. Wire back to Slack API, posting as the authenticated operator. Stand up the MCP server with the three core tools. Test both paths end-to-end: passive (agent posts to Slack → inbox surfaces item) and active (agent calls `atc_request_approval` directly → inbox surfaces item).
...
- **Post as operator, not bot.** ATC sends replies using the operator's token. Non-negotiable.
- **Local-first context graph.** SQLite on the operator's machine. No cloud dependency.
- **No chat interface.** ATC is an inbox with actions, not a chat window.
- **Model-agnostic classifier.** Any OpenAI-compatible endpoint, including Ollama for fully local operation.
- **MCP server from the start.** Agents should be able to push status to ATC directly; don't make passive scraping the only path.
